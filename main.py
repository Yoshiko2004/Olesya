...
def whatsapp(message):
    ...
    if right_word_a:
        ...
        mg = bot.reply_to(message, "Моя твоя не понимать, конечно, но может поговорим о чём-то?", reply_markup=themes)
        bot.register_next_step_handler(mg, choose)
...
def musics(message):
    if message.text == "Продолжить диалог":
        ...
        m_y_n.row(y, n)
        bot.send_message(message.chat.id, text="Мне в целом нравится любая музыка.\nНо слушаю чаще всего какую-то спокойную музыку тк она меня успокаивает и помогает сконцетрироваться.")
        mgm = bot.reply_to(message, "У меня даже ссылка имеется. Тебе скидывать?", reply_markup=m_y_n)
...
def music_url(message):
    if message.text == "Да":
        ...
        link = types.InlineKeyboardButton("🌸", url="https://www.youtube.com/watch?v=l-2hOKIrIyI")
        links.add(link)
        bot.send_message(message.chat.id, "А вот и само видео".format(message.from_user), reply_markup=links)
        ...
        mgm = bot.reply_to(message, "Приятного тебе просмотра".format(message.from_user), reply_markup=themes)
        ...
        themes.row(life, school)
        themes.row(music, preferences)
        mg = bot.reply_to(message, "Ну и ладно", reply_markup=themes)
        bot.register_next_step_handler(mg, choose)
...
def schools(message):
    if message.text == "Что тебе нравится в школе?":
        ...
        bot.reply_to(message, "ПЕРЕМЕНЫ.\nА если серьёзно, то мне заходит литература. Мне она нравится")
        ...
        bot.reply_to(message, "Вот чтобы прям не нравилась, так это физ-ра.\nНа физ-ре мои одноклассники постоянно кидают мяч, а я боюсь этих мячей.\nА ещё я в спорте не очень хороша.")
        msgo = bot.send_message(message.chat.id, text="Какие у тебя вопросы ещё остались?",
                                reply_markup=sch)
        bot.register_next_step_handler(msgo, schools)
...
def preference(message):
    if message.text == "Какую еду ты любишь?":
        ...
        bot.reply_to(message, "В целом я люблю сладкое, но иногда так и хочется скушать кислых конфеток.\n:3")
        ...
        bot.reply_to(message, "Я люблю визуальные новеллы.\nТам красивая графика и плюсом много текста, а я читать люблю.\n:}")
        msgo = bot.send_message(message.chat.id, text="Какие у тебя вопросы ещё остались?",
                                reply_markup=pref)
        bot.register_next_step_handler(msgo, preference)
        ...
        bot.reply_to(message, "Я люблю мультфильмы, если это можно считать жанром фильмом, конечно.\nА так я могу смотреть любе фильмы, главное, чтобы они были хорошими.")
        msgo = bot.send_message(message.chat.id, text="Какие у тебя вопросы ещё остались?",
                                reply_markup=pref)
        bot.register_next_step_handler(msgo, preference)
...
def your_life(message):
    if message.text == "Расскажи о семье.":
        ...
        bot.reply_to(message, "О семье?\nНу... Ладно...\nЯ живу с мамой, младшим сводным братом и отчимом.\nСпросишь где отец?\nТак вот, когда мне было 4 года, отец ушёл за хлебом, но ни отца, ни хлеба нет.\nМама говорит, что он был и пьяницей, и бил её и вообще был не очень хорошим человеком.\nНо сейчас с нами всё хорошо. Мы живём хорошей и дружной семьёй!\n:3")
        ...
        bot.reply_to(message, "Как я поживаю?\nВ целом хорошо. У меня имеется своя комната, родители хорошие и понимающие, брата мне на шею не вешали.\nДа и сам братик хороший, я его люблю, а он меня.\nМне не на что жаловаться.")
        msgo = bot.send_message(message.chat.id, text="Если имеются какие-либо вопросы, спрашивай. Я не против.\n:3",
                                reply_markup=life)
        bot.register_next_step_handler(msgo, your_life)
        ...
        bot.reply_to(message, "Я бы хотела уточнить, что я говорю про летний отдых.\nМы с семьёй часто едем на море.\nБратик даже на компьютере решил нарисовать себя и меня на фоне моря. Говорит, что вдохновился нашим отдыхом на море.")
